...
if (!class_exists('VC_Extensions_ColorBlock')) {
    class VC_Extensions_ColorBlock{
        function VC_Extensions_ColorBlock() {
            ...
            "params" => array(
              ...
              array(
                "type" => "dropdown",
                "holder" => "",
                "heading" => __("Resize the avatar image?", "vc_colorblock_cq"),
                "param_name" => "isresize",
              ...
              array(
                ...
                "group" => "Image",
                "description" => __("Default we will use the original image, specify a width. For example, 1000 will resize the image to width 1000. Please use it carefully, may not work with some server setup.", "vc_colorblock_cq")
              ),
              array(
                "type" => "vc_link",
        ...
        function cq_vc_colorblock_func($atts, $content=null, $tag) {
          ...
          $output .= '<div class="cq-colorblock '.$textblockbg.' '.$elementshape.' '.$extraclass.'" data-titlesize="'.$titlesize.'" data-image="'.$image[0].'" data-contentcolor="'.$contentcolor.'" data-textblockbg="'.$textblockbg.'" data-custombgcolor="'.$custombgcolor.'">';
         ...
         if($isresize=="yes"&&$imagewidth!=""){
              if($image[0]!="") $image_output .= '<img src="'.aq_resize($image[0], $imagewidth, null, true, true, true).'" class="cq-coverslider-image" alt="'.get_post_meta($attachment->ID, '_wp_attachment_image_alt', true ).'">';
          }else{
