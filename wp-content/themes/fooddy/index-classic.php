...
if (substr(fooddy_get_theme_option('blog_style'), 0, 7) == 'masonry') {
...
if (have_posts()) {
	...
	$fooddy_classes = 'posts_container '
						. (substr(fooddy_get_theme_option('blog_style'), 0, 7) == 'classic' ? 'columns_wrap' : 'masonry_wrap');
	...
	if (!$fooddy_sticky_out) {
		if (fooddy_get_theme_option('first_post_large') && !is_paged() && !in_array(fooddy_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
			get_template_part( 'content', 'excerpt' );
		}
		
		?><div class="<?php echo esc_attr($fooddy_classes); ?>"><?php
	}
	while ( have_posts() ) { the_post(); 
		if ($fooddy_sticky_out && !is_sticky()) {
			$fooddy_sticky_out = false;
			?></div><div class="<?php echo esc_attr($fooddy_classes); ?>"><?php
		}
		get_template_part( 'content', $fooddy_sticky_out && is_sticky() ? 'sticky' : 'classic' );
	}
